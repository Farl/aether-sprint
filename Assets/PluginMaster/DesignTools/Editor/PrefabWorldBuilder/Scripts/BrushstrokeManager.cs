...
namespace PluginMaster
{
    ...
    public static class BrushstrokeManager
    {
        ...
        #endregion
        #region FLOOR
        private static void UpdateSingleFloorTileBrushstroke()
        {
            if (PaletteManager.selectedBrush == null) return;
            ...
            var rotation = SnapManager.settings.rotation
                * Quaternion.FromToRotation(Vector3.up, settings.axisAlignedWithNormal);
        ...
        public static void UpdateFloorBrushstroke(bool deleteBox = false)
        {
            ...
            {
                UpdateSingleFloorTileBrushstroke();
                return;
            }
            var settings = FloorManager.settings;
            ...
            var cellsCountX = Mathf.RoundToInt(Mathf.Abs(localDiagonal.x / settings.cellSize.x)) + 1;
            var signX = localDiagonal.x >= 0 ? 1 : -1;
            var dirX = SnapManager.settings.rotation * (Vector3.right * signX);
            var deltaX = dirX * settings.cellSize.x;

            var cellsCountZ = Mathf.RoundToInt(Mathf.Abs(localDiagonal.z / settings.cellSize.z)) + 1;
            var signZ = localDiagonal.z >= 0 ? 1 : -1;
            var dirZ = SnapManager.settings.rotation * (Vector3.forward * signZ);
            var deltaZ = dirZ * settings.cellSize.z;

            var localRotation = Quaternion.FromToRotation(Vector3.up, settings.axisAlignedWithNormal);
            ...
            for(int xIdx = 0; xIdx < cellsCountX; ++ xIdx)
            {
                ...
                for (int zIdx = 0; zIdx < cellsCountZ; ++zIdx)
                {
                    ...
                    if (deleteBox)
                    {
                        var strokeItem = new BrushstrokeItem(null, cellCenter, angle,
                            scaleMultiplier: FloorManager.settings.cellSize, flipX: false, flipY: false, surfaceDistance: 0);
                        _brushstroke.Add(strokeItem);
                    }
                    else
                    {
