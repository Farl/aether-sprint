        ...
        private void TilingGroup()
        {
            ...
            using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
            {
                ...
                using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                {
                    ...
                    settings.cellSizeType = (TilesUtils.CellSizeType)
                        UnityEditor.EditorGUILayout.Popup("Cell size:", (int)settings.cellSizeType, _cellTypeNames);
                    using (new UnityEditor.EditorGUI.DisabledGroupScope(
                        settings.cellSizeType != TilesUtils.CellSizeType.CUSTOM))
                    {
            ...
            using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
            {
                ...
                using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                {
                    ...
                    using (new UnityEditor.EditorGUI.DisabledGroupScope(
                        settings.cellSizeType != TilesUtils.CellSizeType.CUSTOM))
                    {
                        settings.cellSize = UnityEditor.EditorGUILayout.Vector3Field("", settings.cellSize);
                    }
                }
                using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                {
