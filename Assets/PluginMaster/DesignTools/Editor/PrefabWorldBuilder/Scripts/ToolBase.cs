...
namespace PluginMaster
{
    ...
    public static class TilesUtils
    {
        ...
        public static Vector3 GetCellSize(CellSizeType cellSizeType, BrushSettings brush,
            AxesUtils.SignedAxis axisAlignedWithNormal, Vector3 DefaultValue, bool tangentSpace = true)
        {
            if (cellSizeType == CellSizeType.CUSTOM) return DefaultValue;
            if (brush == null) return DefaultValue;

            var cellSize = Vector3.one * (cellSizeType == CellSizeType.SMALLEST_OBJECT
            ...
            if (ToolManager.editMode && PWBIO.selectedPersistentTilingData != null)
            {
                ...
                var scaleMultiplier = cellSizeType == CellSizeType.SMALLEST_OBJECT
                ...
                foreach (var obj in objSet)
                {
                    ...
                    cellSize = cellSizeType == CellSizeType.SMALLEST_OBJECT
            ...
            if (tangentSpace)
            {
                if (axisAlignedWithNormal.axis == AxesUtils.Axis.Y) cellSize.y = cellSize.z;
                else if (axisAlignedWithNormal.axis == AxesUtils.Axis.X)
                {
            ...
            {
                cellSize = Quaternion.FromToRotation(Vector3.up, axisAlignedWithNormal) * cellSize;
