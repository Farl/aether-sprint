                        ...
                        else
                        {
                            using (new GUILayout.HorizontalScope())
                            {
                                ...
                                using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
                                {
                                    ...
                                    if (check.changed && mult != brushSelectionSettings.scaleMultiplier)
                    ...
                    using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                    {
                        ...
                        if (tempBrush.separateScaleAxes)
                        {
                            if (tempBrush.randomScaleMultiplier)
                            {
                                ...
                                if (Mathf.Approximately(range3.x.v1, 0))
                                    range3.x.v1 = tempBrush.randomScaleMultiplierRange.x.v1;
                                if (Mathf.Approximately(range3.x.v2, 0))
                                    range3.x.v2 = tempBrush.randomScaleMultiplierRange.x.v2;

                                if (Mathf.Approximately(range3.y.v1, 0))
                                    range3.y.v1 = tempBrush.randomScaleMultiplierRange.y.v1;
                                if (Mathf.Approximately(range3.y.v2, 0))
                                    range3.y.v2 = tempBrush.randomScaleMultiplierRange.y.v2;

                                if (Mathf.Approximately(range3.z.v1, 0))
                                    range3.z.v1 = tempBrush.randomScaleMultiplierRange.z.v1;
                                if (Mathf.Approximately(range3.z.v2, 0))
                                    range3.z.v2 = tempBrush.randomScaleMultiplierRange.z.v2;

                                tempBrush.randomScaleMultiplierRange = range3;
                            }
                            else
                            {
                                var mult = UnityEditor.EditorGUILayout.Vector3Field(string.Empty,
                                tempBrush.scaleMultiplier);
                                if (Mathf.Approximately(mult.x, 0)) mult.x = tempBrush.scaleMultiplier.x;
                                if (Mathf.Approximately(mult.y, 0)) mult.y = tempBrush.scaleMultiplier.y;
                                if (Mathf.Approximately(mult.z, 0)) mult.z = tempBrush.scaleMultiplier.z;
                                tempBrush.scaleMultiplier = UnityEditor.EditorGUILayout.Vector3Field(string.Empty,
                                tempBrush.scaleMultiplier);
                            }
                        }
                        else
                        {
                            if (tempBrush.randomScaleMultiplier)
                            {
                                var range = EditorGUIUtils.RangeField(string.Empty, tempBrush.randomScaleMultiplierRange.x);
                                if (Mathf.Approximately(range.v1, 0)) range.v1 = tempBrush.randomScaleMultiplierRange.x.v1;
                                if (Mathf.Approximately(range.v2, 0)) range.v2 = tempBrush.randomScaleMultiplierRange.x.v2;
